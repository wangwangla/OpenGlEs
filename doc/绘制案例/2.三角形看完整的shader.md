...
回顾一下绘制过程，写顶点坐标，将顶点数据变为本地数据。然后加载着色器，编译它，创建程序进行关联，进行链接，得到属性应用，使用代码设置数据，然后进行绘制。
...
public class Triangle extends BaseGameScreen {
    private final String vertexShaderCode =
            "attribute vec4 vPosition;" +
                    "void main() {" +
                    "  gl_Position = vPosition;" +  //将位置传递给shader
                    "}";
    ...
    private final String fragmentShaderCode =
            "precision mediump float;" +
                    "uniform vec4 vColor;" +
                    "void main() {" +
                    "  gl_FragColor = vColor;" +  //将颜色给shader
                    "}";
...
- 顶点着色器需要修改，因为上次是uniform参数，这次是将数据放在数组里面。
