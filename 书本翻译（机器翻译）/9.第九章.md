...
渲染3D图形时最基本的操作之一是**将纹理应用到表面**。纹理允许表示仅从网格几何图形中无法获得的附加细节。OpenGL  ES 2.0中的纹理有两种形式:2D纹理和立方体贴图纹理。纹理通常通过使用**纹理坐标**应用于表面，纹理坐标可以被认为是纹理阵列数据的索引。下面几节介绍OpenGL ES中不同的纹理类型，以及如何加载和访问它们。
