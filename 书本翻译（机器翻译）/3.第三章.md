...
**表面有两种方式，一种是自己进行选择合适的，一种是通过指定要求，让EGL自己选择最优的。**
...
除了能够使用OpenGL  ES 2.0渲染到屏幕上的窗口之外，您还可以渲染到不可见的屏幕外表面，称为p buffer(像素缓冲区的简称)。Pbuffers可以充分利用OpenGL ES  2.0可用的任何硬件加速，就像一个窗口一样。Pbuffers最常用于生成纹理贴图。如果你想做的只是渲染一个纹理，我们建议使用帧缓冲区对象(在第12章“帧缓冲区对象”)来代替缓冲区，因为它们更有效。然而，在某些无法使用帧缓冲区对象的情况下，pbuffers仍然很有用，例如用OpenGL  ES渲染屏幕外的表面，然后在另一个API(如OpenVG)中将其用作纹理。
